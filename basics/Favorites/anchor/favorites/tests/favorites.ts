...
import { systemProgramErrors } from "./system-error";
...
describe('Favorites', () => {
  ...
  const favoritesHobbies = ['Binge watching Series', 'Gym'];

  before(async() => {
    const balance = await provider.connection.getBalance(user.publicKey);
    const balanceInSOL = balance/web3.LAMPORTS_PER_SOL;
    const formattedBalance = new Intl.NumberFormat().format(balanceInSOL);
    console.log(`Balance is : ${formattedBalance} SOL`);
  });

  it('Writes our favorites to the blockchain', async() => {
    await program.methods
    .setFavorites(favoriteNumber,favoriteColor, favoritesHobbies)
    .signers([user])
    .rpc();
  ...
  assert.deepEqual(dataFromPda.hobbies, favoritesHobbies);
  });

  it('Updates the favorites', async() => {
    const newFavoriteHobbies = ['skiing', 'skydiving', 'biking', 'swimming'];
    try {
      const signature = await program.methods.setFavorites(favoriteNumber,favoriteColor,newFavoriteHobbies);
